...
struct SyncToProtocol {
	...
	void sync_table(const Table &table) {
		RowReplacer<DatabaseClient> row_replacer(client, table);
		...
		row_replacer.apply(worker.commit_level >= CommitLevel::often, [&] { if (worker.progress) { cout << "." << flush; } });
	...
	void handle_rows_response(const Table &table, RowReplacer<DatabaseClient> &row_replacer) {
		...
		RowRangeApplier<DatabaseClient>(row_replacer, worker.commit_level >= CommitLevel::often, table, prev_key, last_key, [&] { if (worker.progress) { cout << "." << flush; } }).stream_from_input(input);
	}
