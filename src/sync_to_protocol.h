...
struct SyncToProtocol {
	...
	inline void send_hash_command(const Table &table, const KeyRangeToCheck &range_to_check, list<HashResult> &ranges_hashed) {
		...
		worker.client.retrieve_rows(hasher, table, prev_key, last_key, range_to_check.rows_to_hash);
		...
		ranges_hashed.emplace_back(prev_key, last_key, range_to_check.estimated_rows_in_range, range_to_check.priority, hasher.row_count, hasher.size, hasher.finish().to_string(), hasher.last_key);
	}
